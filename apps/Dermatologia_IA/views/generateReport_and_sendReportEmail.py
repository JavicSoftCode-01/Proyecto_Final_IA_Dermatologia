...
<body>
    ...
    <script>
      ...
      if request.GET.get("download") == "true":
        logger.success('GenerateReportView',
                       f'🟢✨ [SUCCESS] Reporte PDF generado y enviado para SkinImage ID {image_id}')
        return pdf_response
      logger.success('GenerateReportView',
                     f'🟢✨ [SUCCESS] Vista de descarga de reporte generada para SkinImage ID {image_id}')
...
class SendReportEmailView(CustomLoginRequiredMixin, ReportActionMixin, View):
  ...
  def post(self, request, image_id):
    ...
    try:
      ...
      if success:
        logger.success('SendReportEmailView',
                       f'🟢✨ [SUCCESS] Reporte enviado exitosamente a {email_address} para SkinImage ID {skin_image_id}')
        ...
        logger.warning('SendReportEmailView',
                       f'🟡⚠️ [WARNING] Fallo al enviar email a {email_address} para SkinImage ID {skin_image_id}')
        return self.handle_report_error(
          request,
